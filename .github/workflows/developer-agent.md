...
4. Mutation modules must:
   - discover current state first;
   - return `changed=false` when desired state already exists;
   - support check mode where practical;
   - perform mutation only when current state differs;
   - return structured results suitable for AAP/Tower.
5. Cluster creation must be safe:
   - if the requested cluster already exists, do not call `New-Cluster`;
   - if a target node is already a member of another cluster, fail safely;
   - if the cluster is absent, validate prerequisites and create it once;
   - rerunning with the same desired state must not recreate or modify the cluster unnecessarily.
...
9. Add or update module documentation and test artifacts for every behavior you introduce.
10. Make the smallest coherent change set that satisfies the issue.

## Required developer handoff

The pull request body must contain a section named `DEVELOPER HANDOFF` with:
...
- exact tests the Tester Agent should execute.
