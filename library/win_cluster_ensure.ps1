  ...
  if ($null -ne $cluster) {
    ...
    $currentIps = @(
      Get-ClusterResource -Cluster $module.Params.name |
        ...
        ForEach-Object { (Get-ClusterParameter -InputObject $_ -Name Address -ErrorAction SilentlyContinue).Value } |
        Where-Object { $_ }
    )

    if (Compare-Object $currentNodes $desiredNodes) {
      $module.FailJson("Cluster exists but node membership differs. Current: $($currentNodes -join ', ')")
    }
    if ($currentIps -notcontains $module.Params.static_address) {
      $module.FailJson("Cluster exists but static IP differs. Current: $($currentIps -join ', ')")
    }
